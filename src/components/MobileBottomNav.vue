  ...
  <nav class="lg:hidden fixed bottom-0 left-0 right-0 z-40 theme-panel-strong backdrop-blur-xl border-t theme-border theme-safe-bottom">
    <div class="flex items-stretch h-16">
      <router-link
        ...
        :key="item.path"
        :to="item.path"
        class="flex-1 flex flex-col items-center justify-center gap-1 text-xs transition-all min-h-[56px] active:scale-95"
...
const navItems = computed(() => {
  const items = [
    ...
    { path: userAuthStore.isAuthenticated ? '/me/orders' : '/guest/orders', icon: 'orders', label: 'bottomNav.orders' },
