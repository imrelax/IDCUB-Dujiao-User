  ...
  <div
    ...
    <div class="p-3 md:p-4 relative z-20 flex flex-col flex-1">
      ...
      <div class="mb-2 md:mb-3 flex flex-wrap items-center gap-1 md:gap-2">
        <!-- Product Badge (热销/推荐/爆款 etc.) -->
        <span v-if="productBadge"
          class="theme-badge px-2 md:px-3 py-0.5 md:py-1 text-xs font-bold text-white"
          :style="{ background: productBadge.gradient }">
          {{ productBadge.label }}
        </span>
...
const productBadge = computed(() => {
  ...
  const badge = getProductBadge(props.product.id)
  if (!badge) return null
  return {
    ...badge,
    label: resolveLocalizedText(badge.label, locale.value)
  }
})
</script>
