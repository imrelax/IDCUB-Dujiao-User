  ...
  <Teleport to="body">
    ...
    <Transition
      ...
      <div v-if="showMobileMenu"
        class="lg:hidden fixed right-0 top-0 bottom-0 z-[70] w-80 max-w-[85vw] theme-panel-strong backdrop-blur-xl border-l theme-border overflow-y-auto"
        style="overscroll-behavior: none;">
        <div class="p-4 space-y-2">
          <!-- Header -->
          <div class="flex items-center justify-between mb-6 px-2">
            <span class="text-sm font-bold theme-text-primary">{{ t('navbar.menu') }}</span>
            <button @click="showMobileMenu = false" class="p-2.5 min-w-[48px] min-h-[48px] flex items-center justify-center rounded-xl theme-btn-neutral active:scale-95 transition-transform">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <template v-for="item in mobileDrawerItems" :key="item.key">
            <router-link v-if="item.type === 'route'" :to="item.path" @click="showMobileMenu = false"
              class="block w-full text-left px-4 py-3.5 rounded-xl theme-nav-link text-base min-h-[52px] flex items-center gap-4 active:scale-[0.98] transition-transform"
              active-class="theme-nav-link-active bg-gray-100 dark:bg-white/10">
              <svg class="w-5 h-5 shrink-0 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" :d="item.icon" />
              </svg>
              <span class="font-medium">{{ item.label.startsWith('nav.') ? t(item.label) : item.label }}</span>
            </router-link>
          ...
          <!-- Divider -->
          <div class="h-px theme-border my-4"></div>
          ...
          <router-link v-if="!userAuthStore.isAuthenticated" to="/guest/orders" @click="showMobileMenu = false"
            ...
            <span class="font-medium">{{ t('navbar.guestOrders') }}</span>
          </router-link>

          <!-- Logout -->
          <button v-if="userAuthStore.isAuthenticated" @click="userAuthStore.logout(); showMobileMenu = false"
            class="w-full text-left px-4 py-3.5 rounded-xl text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors font-medium min-h-[52px] flex items-center gap-4 active:scale-[0.98]">
            <svg class="w-5 h-5 shrink-0 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
            </svg>
            <span class="font-medium">{{ t('navbar.logout') }}</span>
          </button>

          <!-- Language Switcher -->
          <div class="mt-4 pt-4 border-t theme-border">
            <button @click="showMobileLangMenu = !showMobileLangMenu"
              class="w-full flex items-center justify-between px-4 py-3 rounded-xl theme-nav-link text-base min-h-[48px] active:scale-[0.98] transition-transform">
              <div class="flex items-center gap-3">
                ...
                <span class="font-medium">{{ t('navbar.selectLanguage') }}</span>
              </div>
              <svg class="w-4 h-4 opacity-50 transition-transform" :class="showMobileLangMenu ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            <div v-if="showMobileLangMenu" class="mt-1 space-y-1 pl-4">
              <button v-for="lang in languages" :key="lang.code" @click="changeLanguage(lang.code)"
                class="w-full text-left px-4 py-2.5 rounded-xl text-sm transition-colors min-h-[44px] flex items-center justify-between active:scale-[0.98]"
                :class="appStore.locale === lang.code
                  ? 'theme-text-accent font-semibold bg-gray-100 dark:bg-white/10'
                  : 'theme-text-secondary hover:theme-text-primary'">
                <span>{{ lang.name }}</span>
                <span v-if="appStore.locale === lang.code"
                  class="w-2 h-2 rounded-full theme-accent-stick"></span>
              </button>
            </div>
          </div>
        </div>
...
const builtinNavDefs: Record<string, { path: string; label: string; icon: string }> = {
  ...
  tutorials: { path: '/tutorials', label: 'nav.tutorials', icon: 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253' },
...
const buildBuiltinNavItems = (): NavItem[] => {
  ...
  for (const [key, def] of Object.entries(builtinNavDefs)) {
    ...
    if (key === 'tutorials' && (xiabibiConfig as any).tutorialEnabled?.enabled === false) continue
...
const menuItems = computed<NavItem[]>(() => {
  const items: NavItem[] = [
    ...
    { key: 'categories', path: '/categories', label: 'nav.categories', icon: 'M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z', type: 'route', target: '_self' },
  ]
  if (!isListMode.value) {
...
// Mobile drawer shows all menu items for easy access
