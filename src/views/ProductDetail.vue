    ...
    <div class="container mx-auto px-4">
      ...
      <div v-else-if="product">
            ...
            <nav class="mb-8 flex items-center space-x-2 text-sm theme-text-muted font-medium">
              ...
              <span>/</span>
              <router-link to="/products" class="theme-link-muted">{{ t('nav.products') }}</router-link>
              <span>/</span>
              <span class="theme-text-primary truncate max-w-[200px]">{{ getLocalizedText(product.title) }}</span>
            </nav>

            <!-- Main Info Card (Top Full Width) -->
            <div class="theme-panel backdrop-blur-xl border rounded-3xl overflow-hidden mb-8 shadow-2xl">
            ...
            <div class="p-6 md:p-8 lg:p-12 flex flex-col justify-center">
              <div class="mb-6">
                ...
                <div class="mb-6 flex flex-wrap items-center gap-2">
                  ...
                  <span class="theme-badge theme-badge-neutral">
                    ID: {{ product.id }}
                  </span>
                </div>

                <div class="mb-8 border-b theme-border pb-8" ref="priceSection">
                  <div v-if="marketingInfo" class="marketing-price-container rounded-xl overflow-hidden" :style="marketingGradientStyle">
                    <div class="marketing-title-bar px-4 py-2">
                      <span class="text-sm font-bold text-white">{{ marketingTitle }}</span>
                    </div>
                    <div class="marketing-price-area px-4 py-3">
                      <div v-if="selectedSku && hasSkuPromotionPrice(selectedSku)" class="space-y-2">
                        <div class="flex flex-wrap items-end gap-4">
                          <span class="text-sm theme-text-muted shrink-0 leading-[2.75rem]">{{ t('products.price') }}</span>
        ...
        <div class="flex flex-col lg:flex-row gap-8 items-stretch">
          ...
          <aside class="w-full lg:w-64 shrink-0 hidden lg:block">
            <CategorySidebar
              ...
              @toggle-parent="handleToggleParentCategory"
          ...
          <div class="flex-1 min-w-0 flex flex-col">
            <div v-if="product.content"
              class="theme-panel backdrop-blur-xl border rounded-3xl overflow-hidden p-6 md:p-8 lg:p-12 relative flex-1">
              <h2
...
import { getMarketingTitle, resolveLocalizedText } from '../utils/productBadge'
...
const marketingInfo = computed(() => {
  if (!product.value) return null
  return getMarketingTitle(product.value.id, locale.value)
})

const marketingTitle = computed(() => {
  if (!marketingInfo.value) return ''
  return resolveLocalizedText(marketingInfo.value.label, locale.value)
})

const marketingGradientStyle = computed(() => {
  if (!marketingInfo.value) return {}
  const gradient = marketingInfo.value.gradient
  const colorMatch = gradient.match(/#[0-9a-fA-F]{6}/g)
  const darkColor = colorMatch ? colorMatch[0] : '#ff4d4f'
  const lightColor = colorMatch && colorMatch.length > 1 ? colorMatch[1] : darkColor
  return {
    '--marketing-gradient': gradient,
    '--marketing-dark': darkColor,
    '--marketing-light': lightColor,
  }
})
...
// Sidebar Categories
const listCategoryGroups = ref<any[]>([])
const productCategoryId = computed(() => product.value?.category?.id || null)
const listExpandedParentIds = ref<number[]>([])
const listShowFilterDrawer = ref(false)

const handleSelectCategory = (categoryId: number | null, closeDrawer = false) => {
  if (closeDrawer) {
    ...
    const findSlug = (groups: any[]) => {
      for (const group of groups) {
        ...
        if (group.children && group.children.length > 0) {
          if (findSlug(group.children)) return true
        }
      }
      return false
    }
    findSlug(listCategoryGroups.value)
...
const handleToggleParentCategory = (categoryId: number) => {
  if (listExpandedParentIds.value.includes(categoryId)) {
    listExpandedParentIds.value = listExpandedParentIds.value.filter(id => id !== categoryId)
  } else {
...
onMounted(async () => {
  ...
  try {
    ...
    listCategoryGroups.value = buildCategoryGroups(res.data.data || [])
  } catch (error) {
