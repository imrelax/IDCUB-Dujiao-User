<template>
  <div class="home-page min-h-screen theme-page">
    ...
    <!-- Full-width Slider -->
    <section class="relative w-full h-[300px] sm:h-[400px] md:h-[500px] lg:h-[600px] overflow-hidden mt-[85px] mb-0">
      <Transition name="slider-fade">
        <div
          :key="currentSlideIndex"
          class="absolute inset-0 bg-gradient-to-r"
          :class="currentSlide.bgColor"
        >
          <div class="container mx-auto h-full px-4 md:px-8">
            <div class="flex h-full items-center justify-between">
              <!-- Left Content -->
              <div class="w-full md:w-1/2 z-10 space-y-3 sm:space-y-4 md:space-y-6">
                <span v-if="currentSlide.subtitle" class="inline-block py-1.5 px-4 rounded-full bg-white/20 backdrop-blur-sm text-white text-xs sm:text-sm font-semibold tracking-wider uppercase border border-white/30">
                  {{ currentSlide.subtitle }}
                </span>
                <h2 class="text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-black text-white drop-shadow-lg leading-tight">
                  {{ currentSlide.title }}
                </h2>
                <p class="text-sm sm:text-base md:text-xl text-white/90 max-w-xl drop-shadow leading-relaxed whitespace-pre-line line-clamp-3 sm:line-clamp-none">
                  {{ currentSlide.description }}
                </p>
                <div class="pt-2 sm:pt-4" v-if="currentSlide.buttonText">
                  <button @click="handleSlideClick(currentSlide)" class="bg-white text-gray-900 hover:bg-gray-50 px-6 sm:px-8 py-3 sm:py-3.5 rounded-full font-bold shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 active:scale-95 flex items-center gap-2 text-sm sm:text-base">
              ...
              <div class="hidden md:flex w-1/2 h-full items-center justify-end z-10 opacity-80 hover:opacity-100 transition-opacity duration-500 cursor-pointer" @click="handleSlideClick(currentSlide)">
                <div class="w-64 h-64 lg:w-96 lg:h-96 text-white drop-shadow-2xl" v-html="currentSlide.svg"></div>
              </div>
            </div>
          </div>
          ...
          <div class="absolute bottom-0 left-0 -ml-20 -mb-20 w-80 h-80 rounded-full bg-black/10 blur-3xl pointer-events-none"></div>
        </div>
      </Transition>
      ...
      <button
        @click="prevSlide"
        class="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 w-12 h-12 md:w-14 md:h-14 rounded-full bg-black/40 hover:bg-black/60 text-white flex items-center justify-center transition-all backdrop-blur-sm active:scale-95"
      ...
      <button
        @click="nextSlide"
        class="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 w-12 h-12 md:w-14 md:h-14 rounded-full bg-black/40 hover:bg-black/60 text-white flex items-center justify-center transition-all backdrop-blur-sm active:scale-95"
      ...
      <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-3">
        <button
          v-for="(_, index) in sliderSlides"
          :key="index"
          @click="goToSlide(index)"
          class="w-4 h-4 md:w-3 md:h-3 rounded-full transition-all active:scale-90"
          :class="currentSlideIndex === index ? 'bg-white w-8 md:w-8' : 'bg-white/50 hover:bg-white/70'"
    ...
    <!-- Notice Bar -->
    <section v-if="noticeBarVisible && notices.length > 0" class="theme-surface-soft border-b theme-border">
      <div class="container mx-auto px-4 py-3">
        <div class="flex items-center gap-3 overflow-hidden">
          <span class="flex-shrink-0 inline-flex items-center gap-1.5 text-xs font-semibold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 px-2.5 py-1 rounded-full">
          ...
          <div class="flex-1 overflow-hidden relative h-5">
            <Transition name="notice-scroll" mode="out-in">
              <div
                :key="currentNoticeIndex"
                class="flex items-center gap-2 cursor-pointer absolute inset-0"
                @click="goToNoticeDetail(notices[currentNoticeIndex]?.slug)"
              >
                <span class="text-sm theme-text-primary truncate">
                  {{ getLocalizedText(notices[currentNoticeIndex]?.title) }}
                </span>
              </div>
            </Transition>
          </div>
          <router-link
            to="/notice"
            class="flex-shrink-0 text-xs font-medium theme-link-muted hover:theme-link transition-colors"
          >
            查看全部
          </router-link>
        </div>
    ...
    <template v-if="templateMode === 'list'">
      ...
      <section class="relative z-10 pt-[40px] pb-[40px]">
        <div class="container mx-auto px-4">
    ...
    <section id="featured" class="relative z-10 pb-8" :class="showHeroSection ? 'pt-6' : 'pt-32 md:pt-36'">
      <div class="container mx-auto px-4">
...
import { sortProductsByBadge } from '../utils/productBadge'
...
import sliderData from '../assets/xiabibi.json'
...
const sliderSlides: SliderSlide[] = (sliderData.slides as any)?.items || []

const currentSlideIndex = ref(0)
let slideAutoPlayTimer: ReturnType<typeof setInterval> | null = null

const currentSlide = computed(() => sliderSlides[currentSlideIndex.value] ?? sliderSlides[0]!)

const nextSlide = () => {
  currentSlideIndex.value = (currentSlideIndex.value + 1) % sliderSlides.length
}

const prevSlide = () => {
  currentSlideIndex.value = (currentSlideIndex.value - 1 + sliderSlides.length) % sliderSlides.length
}
...
const handleSlideClick = (slide: SliderSlide) => {
  if (slide.link) {
...
let noticeAutoPlayTimer: ReturnType<typeof setInterval> | null = null

const loadNotices = async () => {
  try {
    const noticeSettings = (sliderData as any).noticeSettings || {}
    if (noticeSettings.enabled === false) {
      notices.value = []
      return
    }
    ...
    const configuredIds = noticeSettings.noticeIds || []
    if (configuredIds.length > 0) {
      notices.value = allNotices.filter((n: any) => configuredIds.includes(Number(n.id)))
    } else {
      notices.value = allNotices.slice(0, 5)
    }
  } catch (error) {
...
const startNoticeAutoPlay = () => {
  if (noticeAutoPlayTimer) {
    clearInterval(noticeAutoPlayTimer)
  }
  if (notices.value.length > 1) {
    noticeAutoPlayTimer = setInterval(() => {
      currentNoticeIndex.value = (currentNoticeIndex.value + 1) % notices.value.length
    }, 4000)
  }
}

const stopNoticeAutoPlay = () => {
  if (noticeAutoPlayTimer) {
    clearInterval(noticeAutoPlayTimer)
    noticeAutoPlayTimer = null
  }
}

const goToNoticeDetail = (slug: string) => {
  if (slug) {
...
const loadFeaturedProducts = async () => {
  try {
    ...
    const hideOutOfStock = (sliderData as any).hideOutOfStock?.enabled !== false
    const filtered = hideOutOfStock
      ? rawProducts.filter((p: any) => p.stock_status !== 'out_of_stock')
      : rawProducts
    products.value = sortProductsByBadge(filtered)
  } catch (error) {
...
onMounted(async () => {
  startSlideAutoPlay()
  loadNotices().then(() => startNoticeAutoPlay())
...
.notice-scroll-enter-active,
.notice-scroll-leave-active {
  transition: all 400ms ease;
}
.notice-scroll-enter-from {
  opacity: 0;
  transform: translateY(-100%);
}
.notice-scroll-leave-to {
  opacity: 0;
  transform: translateY(100%);
}
</style>
